...
int main(int argc, char* argv[]) {
    ...
    // Create and start threads, assigning each a specific range of numbers
    for (unsigned int i = 0; i < numThreads; ++i) {
        int end;
        // Set the thread's range
        if (i == numThreads - 1) {
            end = limit;
        } else {
    ...
    // Sort the final list of primes to ensure correct order
    std::sort(primes.begin(), primes.end());
    
    // Output the computed prime numbers
